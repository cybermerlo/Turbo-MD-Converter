...
class TurboMDConverterApp(ctk.CTk, TkinterDnD.DnDWrapper):
    ...
    def _open_whatsapp_import(self) -> None:
        ...
        WhatsAppImportWindow(self, on_chat_ready=self._on_whatsapp_chat_ready)

    def _on_whatsapp_chat_ready(self, md_path: Path) -> None:
        self.input_frame.add_paths([md_path])
        self.log_frame.append(f"Chat WhatsApp importata: {md_path.name}")
